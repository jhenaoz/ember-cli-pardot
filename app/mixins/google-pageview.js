...
export default Ember.Mixin.create({
  ...
  pageviewToGA: Ember.on('didTransition', function(page, title) {
    ...
    if (Ember.get(ENV, 'pardotAnalytics.webPropertyId') != null) {
      var trackerType = Ember.getWithDefault(ENV, 'pardotAnalytics.tracker', 'analytics.js');

      if (trackerType === 'analytics.js') {
        var globalVariable = Ember.getWithDefault(ENV, 'pardotAnalytics.globalVariable', 'ga');
